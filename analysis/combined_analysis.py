...
emis = pd.read_csv(
    "released_outputs/EMIS/doac_rate_total.csv", usecols=["population", "doac", "date"]
)
tpp = pd.read_csv(
    "released_outputs/TPP/doac_rate_total.csv", usecols=["population", "doac", "date"]
)
emis_recent = pd.read_csv(
    "released_outputs/EMIS/doac_rate_total_recent.csv",
    usecols=["population", "doac", "date"],
)
tpp_recent = pd.read_csv(
    "released_outputs/TPP/doac_rate_total_recent.csv",
    usecols=["population", "doac", "date"],
)
...
plot_measures(
    combined,
    "Number of people with coded as having a mechanical valve prescribed a DOAC",
    "doac",
    "count",
    y_label="Count",
)
plot_measures(
    combined,
    "Rate of DOAC prescriptions per 1000 patients coded as having a mechanical valve",
    "rate",
    "rate",
...
def plot_measures_with_means(
    df, title, column_to_plot, filename, mean_1, mean_2, y_label="Rate per 1000"
):
    ...
    plt.errorbar(2.5, mean_1, yerr=ci_1, c="red", ls="none", capsize=4)

    plt.errorbar(8.5, mean_2, yerr=ci_2, c="red", ls="none", capsize=4)
...
plot_measures_with_means(
    combined,
    "Number of people coded as having a mechanical valve prescribed a DOAC",
    "doac",
    "count_with_mean",
    y_label="Count",
